...
- **[`bcm/`](./bcm/)** — BCM 方法、23 家真實公司實作範例、參考範本。
...
## Q：有做好的 BCM 範例嗎？
`bcm/` 累積 23 家**真實公司**，各一份「輸入功課 HTML」＋一張「BCM 圖」。橫跨製造（TSMC、BASF）、品牌零售（L'Oréal、Pandora、The RealReal）、訂閱內容（Netflix、NYT）、平台與遊戲（CMoney、PoE2）、服務代理（We Are Social）、非營利研究法人（工研院 ITRI）、分發基建與廣告分發（Cloudflare、TollBit、The Trade Desk、AppLovin）、內容分發（ByteDance、小紅書、SmartNews、OpusClip）、廣告代理與行銷服務（Publicis、WPP、Omnicom、Accenture Song）。各家差異化落點不同，方法照各家研究推導、非套版。完整對照表與每家視角見 [`bcm/README.md`](./bcm/README.md)。
